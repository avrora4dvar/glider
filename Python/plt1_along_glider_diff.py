...
for (mat1,ax1) in zip(mat['model'],ax[1:]):
    ...
    z1=np.reshape(mat['obs'].depth,(1,-1))+np.zeros(np.shape(mat['obs'].temp))
    val1=mat1.salt-valS
    cplot1=ax1.contourf(t1,z1,val1,levels=np.arange(-5,5.01,.2),cmap=cm,extend='both')

    t1=np.reshape(mat['obs'].t,(-1,1))+np.zeros(np.shape(mat['obs'].temp))
    z1=np.reshape(mat['obs'].depth,(1,-1))+np.zeros(np.shape(mat['obs'].temp))
    val1=mat1.salt-valS
    cplot2=ax1.contour(t1,z1,val1,linestyles=':',linewidths=.7,
            colors='k',levels=np.arange(-4,0,.5))
    cplot2=ax1.contour(t1,z1,val1,linestyles='--',linewidths=.7,
            colors='k',levels=np.arange(.5,4.01,.5))
    cplot2=ax1.contour(t1,z1,val1,linestyles='-',linewidths=.7,
            colors='k',levels=[0])
...
fig.savefig('along_glider_diffS.pdf')

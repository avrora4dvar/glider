...
nccreate(out_file,sprintf('%s_time','lrf'),'Format','netcdf4_classic',...
    'Dimensions',{'time',Inf}); 
nccreate(out_file,sprintf('%s','lwrad_down'),'Format','netcdf4_classic',...
    'Dimensions',{'xi_rho',gsize(1),'eta_rho',gsize(2),'time',Inf}); 

%write time
ncwrite(out_file,sprintf('%s_time','lrf'),tList-datenum('2005-01-01')); 
...
for it=1:length(tList)
    ...
    %read Grib file
    val=nco{'downward_long_wave_rad_flux_surface'}(1,grib.mask1,grib.mask2)/100; 
    val=squeeze(val); 
    ...
    %write to output
    ncwrite(out_file,'lwrad_down',valp2,[1,1,it]);     
